...
export const autoGenerateDefense = (
  ...
  const minimumMissingEarlyInfield = Math.max(0, players.length - (firstThreeInnings * POSITION_GROUPS.INFIELD.length));
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    ...
    for (let i = 1; i <= inningsCount; i++) {
      ...
      const chooseCandidates = (pos: string): Player[] => {
        ...
        const scored = hardCandidates.map((p) => {
          ...
          const needsEarlyInfield = i <= 3 && !playerEarlyInfield.get(p.id);
    ...
    const score = validation.errors.length * 10000 + missingEarlyInfield * 25 + nonDugoutWarnings * 50 + dugoutWarnings;
