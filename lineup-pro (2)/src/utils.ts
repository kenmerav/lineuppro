...
export const autoGenerateDefense = (
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    ...
    for (let i = 1; i <= inningsCount; i++) {
      ...
      for (const pos of positionsThisInning) {
        ...
        const noRepeatNoHistory = hardCandidates.filter((p) =>
          !playerPositionsPlayed.get(p.id)?.has(pos) && !historicalPositions.get(p.id)?.has(pos)
        );
        const noRepeat = hardCandidates.filter((p) => !playerPositionsPlayed.get(p.id)?.has(pos));
        const pool =
          noRepeatNoHistory.length > 0 ? noRepeatNoHistory :
          noRepeat.length > 0 ? noRepeat :
          hardCandidates;
    ...
    const score = validation.errors.length * 1000 + nonDugoutWarnings * 50 + dugoutWarnings;
    ...
    if (validation.errors.length === 0 && nonDugoutWarnings === 0) {
      return assignments;
    }
  }
